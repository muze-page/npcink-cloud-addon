...
function npcink_cloud_addon_wporg_fail( $message ) {
	global $failures;
	$failures[] = (string) $message;
}

function npcink_cloud_addon_wporg_read( $path ) {
	if ( ! is_readable( $path ) ) {
		npcink_cloud_addon_wporg_fail( 'Missing readable file: ' . $path );
		return '';
	}
...
function npcink_cloud_addon_wporg_php_files() {
	global $root;
...
foreach ( npcink_cloud_addon_wporg_php_files() as $file ) {
	$relative = str_replace( $root . '/', '', $file );
	$contents = npcink_cloud_addon_wporg_read( $file );

	foreach ( $rules as $message => $pattern ) {
		if ( preg_match( $pattern, $contents ) ) {
			npcink_cloud_addon_wporg_fail( $relative . ': ' . $message );
		}
	}
}
